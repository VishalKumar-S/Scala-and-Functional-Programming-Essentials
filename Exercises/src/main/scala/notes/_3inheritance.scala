...
object _3inheritance extends App {
  val p = new Parent()
  ...
  sp.playMusic()
  sp.trimMusic()
  println("In Scala type hierarchy, scala.any is the root of all types, it contains ==, equlas, hashcode,toString, its direct subtypes/subclasses are AnyVal (parent of all primitive data types) and AnyRef (parent of all reference types, its equivalent to java.lang.Object, it includes all scala classes, java classes, collections,tuples, string)\n scala.Null derives/extends from scala.AnyRef, so that is why for any ref type, u can replace what it's pointing to, to Null, without any issues, since Null is a subtype of all the classes, the ref type is pointing to. \n scala.Nothing derives/inherits from both scala.AnyVal as well as scala.Null, (i.e) it is the bottom most subclasses, its inheriting all the classes used in Scala. nothing is used in exceptions. nothing does refers to instance of any class. nothign means nothing, it is of no type. ")
...
trait Camera {
  println("Traits are a type in Scala, here also both abstract and non abstract methods can be present, like abstract classes, but the difference is one class, can inherit from only 1 class, but it can inherit from multiple traits. So,it acts as multiple inheritance.")
  def takePhoto(): Unit
  def editImages(): Unit
}
...
class SmartPhone extends Phone("Samsung","Jio", 123456789) with Camera with MusicPlayer {
  println("Overriding abstract methods of an abstract class/trait on its subclass, using overrided keyword is optional only.")
...
sealed class Parent() {
  ...
  final def superKeyword() = println("I can't be overriden by any classes, since im a final method. 'super' keyword is used to access the parent's fields/members. It wsi sued inc ases, where u don't want to use the overriden child field members, instead use parent's fields")
}

final class Child(override val content: String) extends Parent("In Scala, whenever u try to execute a child class, before executign the child class constructor, if its inherits any parent, it's parents constructors r checked first, before child class constructors, if the child class constructos correctly inherit the valid constructor of parent class(either primary constructor or auxillary constructors, then only , it compiles, for e.g, here we can inherit class Child extends Parent {} \n class Child extends Parent(a: String) {}\n It would execute fine, but if, we execute like class Child extends Parent(a: String, b:int) {} => it will lead to error, since there is no valid constructor of this type in the parent class.") {
